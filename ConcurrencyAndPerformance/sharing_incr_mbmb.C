...
// Compares data-sharing performance for simple atomic, mutex (lock-based), CAS (compare-and-swap), 
// custom spinlock and pointer spinlock synchronization mechanisms 
...
class Spinlock {
    ...
    void lock () {
        ...
        for (int i=0; flag_.load(std::memory_order_relaxed) || flag_.exchange(1, std::memory_order_relaxed); ++i) {
            if (i == 8) {
                i = 0; 
                nanosleep(&ns, NULL); 
            }
...
#define ARGS ->ThreadRange(1, numcpu) ->UseRealTime()
