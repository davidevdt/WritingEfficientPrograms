...
void BM_loop_int(benchmark::State& state) {
    ...
    const unsigned int N = state.range(0);      // N will be 1<<20
...
// To compile: 
// $ g++ microbenchmarking.C -lbenchmark -lpthread -o microbenchmarking 
// $ ./microbenchmarking
// $ ./microbenchmarking --benchmark_report_aggregates_only=true --benchmark_repetitions=10